...
bool Eeprom::getFlagInEeprom(vmemory_index index, int boolSlot)
{
...
void Eeprom::setFlagInEeprom(vmemory_index index, bool* boolSlots)
{
  byte buffer;
  for (int i = 0; i < 8; i++) {
...
char* Eeprom::getPasswordInEeprom(byte index)
{
  char buffer[32];
...
void Eeprom::setPasswordInEeprom(byte index, char* password)
{

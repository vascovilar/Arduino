...
bool ST7789SD::init()
{
  ...
  // SD card
  /*if (!_sd.begin(_sdcsPin)) { // SD_SCK_MHZ(10) Breakouts require 10 MHz limit due to longer wires
...
//bgColor = COLOR_RED; // debug
  ...
  if (isFixedWidthFont) {
    ...
    _tft.setCursor(x, y + VTFT_CHAR_HEIGHT * _convertToSize(size));
...
void ST7789SD::image(float x, float y, const uint16_t* data, int width, int height)
{
  //_tft.drawXBitmap(x, y, data, width, height, _convertToRGB565(COLOR_BLACK), _convertToRGB565(COLOR_BLUE));
...
float ST7789SD::width(String content, vtextsize size, bool isFixedWidthFont)
{
  ...
  if (isFixedWidthFont) {
    ...
    width = content.length() * VTFT_CHAR_WIDTH * _convertToSize(size);
  } else {
    if (size != SIZE_NULL) {
      _tft.setTextSize(_convertToSize(size));
      _tft.setFreeFont(_gfxFont);
...
float ST7789SD::height(String content, vtextsize size)
{
  ...
  return (VTFT_CHAR_HEIGHT + 1) * multiplier; // ever 1 line height
}

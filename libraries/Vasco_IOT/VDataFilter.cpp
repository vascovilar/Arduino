...
int VDataFilter::_isometric(float value, float maximum, float minimum, int height, int offset) 
{
  return int((1 - ((float) (value - minimum) / (float) (maximum - minimum))) * height) + offset;
}

float VDataFilter::_smooth(float value, int factor) 
{
...
float VDataFilter::_maximum(float value, int factor) 
{
  if (factor >= _BUFFER_MAX_SIZE) {
    Serial.print(F("Can't smmoth, factor must be < "));
    Serial.println(_BUFFER_MAX_SIZE);
...
float VDataFilter::_inertia(float value, int factor) 
{
  if (factor >= _BUFFER_MAX_SIZE) {
    Serial.print(F("Can't smmoth, factor must be < "));
    Serial.println(_BUFFER_MAX_SIZE);

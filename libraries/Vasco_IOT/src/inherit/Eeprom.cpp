...
bool Eeprom::_getFlagInEEPROM(vmemory_index index, int boolSlot)
{
...
void Eeprom::_setFlagInEEPROM(vmemory_index index, bool* boolSlots)
{
  byte buffer;
  for (int i = 0; i < 8; i++) {
...
char* Eeprom::_getPasswordInEEPROM(byte index)
{
  char buffer[32];
...
void Eeprom::_setPasswordInEEPROM(byte index, char* password)
{

...
bool VUseAdcPin::_initADC(byte attachedPin, bool isAmplified, float maxAnalogValue, float zeroAnalogValue) 
{
...
float VUseAdcPin::_readADCFrequency()
{
  long time = millis();
  long timer = micros();
  float buffer[100];
...
bool VUsePwmPin::_initPWM(byte attachedPin, byte channel) 
{
  if ((attachedPin >= 0 && attachedPin <= 5) // for ESP32  // go TODO vasco not in array 
...
void VUsePwmPin::_ledPWM(int magnitude) // 0~4096
{
  if (magnitude > 4096) {
...
void VUsePwmPin::_ledPWM(bool onOrOff)
{
  pinMode(_attachedPin, OUTPUT);
  digitalWrite(_attachedPin, (int) onOrOff);
}

void VUsePwmPin::_tonePWM(int frequency) 
{

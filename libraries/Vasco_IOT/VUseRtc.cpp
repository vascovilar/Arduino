...
void VUseRtc::_setTimeRTC(long timeStamp) 
{
...
String VUseRtc::_getDateTimeRTC()
{
  struct tm timeinfo;
  time_t now;
  time(&now);
...
void VUseRtc::_setDateTimeRTC(String dateTime)
{
  struct tm timeinfo;
  ...
  _setTimeRTC(timeSinceEpoch);
}

String VUseRtc::_getUpTimeRTC()
{
  long time = millis();
  int sec = time / 1000;
  int min = sec / 60;
